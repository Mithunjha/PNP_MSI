...
def cg_solve(A_func, A_t_func, sampling_pattern, b, rho, v, N,  tol=1e-6, max_iter=200):#max_iter = 200; 5 was used  in the original code#1e-6
    ...
    x, _ = scipy.sparse.linalg.cg(lhs, rhs, tol=tol, maxiter=max_iter)
    return x.reshape((N,N))




def admm_inverse_denoising(A_func, A_t_func, pattern, b, test_data_gt, rhos, tol, experiment_name, N,  denoiser, max_iters=50, mask=None, device = None):
    W, H = N, N
    x_old = np.zeros((W,H))
    z_old = np.zeros((W, H))#np.zeros((W, H, 2))
    u_old = np.zeros((W, H))
